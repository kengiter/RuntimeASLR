...
// RuntimeASLR: generate a random and available base address
#define BASE_MASK 0x7ffffffff000
unsigned long get_rand_base(unsigned int size) {
  unsigned long base = 0x7f0000000000;
  int f_rand = __open("/dev/urandom", 0);
  while (1) {
    int n_read = __read(f_rand, ((unsigned char *)&base) + 1, 4);
    if (!n_read)
      continue;
    base = base & BASE_MASK;
    unsigned long tmp_base = (unsigned long)__mmap(
        (caddr_t)base,
        size,
        0,
...
cannot allocate TLS data structures for initial thread");
  ...
  if (mappref == 0) {
	  l->l_map_start = (ElfW(Addr)) __mmap ((void *) get_rand_base(maplength), maplength,
					        c->prot,
